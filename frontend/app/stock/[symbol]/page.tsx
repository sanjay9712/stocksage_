...
import { fetchStockDetail, fetchCandlestick, fetchMultiFactor } from "@/lib/api";
...
export default function StockDetailPage({ params }: { params: { symbol: string } }) {
  ...
  const { data, isLoading, error } = useSWR(`stock-${symbol}`, () => fetchStockDetail(symbol), { refreshInterval: 2000, keepPreviousData: true });
    ...
    <div className="space-y-5 fade-in">
      ...
      <div className="glass-card p-5">
        ...
        <div className="flex items-center gap-4 mt-3">
          ...
          {data.intraday_pick && (
            <a href={`/picks/${symbol}`} className="text-xs text-emerald-400 hover:underline">
              View today&apos;s intraday pick →
            </a>
          )}
        </div>
