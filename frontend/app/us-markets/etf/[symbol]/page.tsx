...
import { fetchUsEtfDetail } from "@/lib/api";

function pct(v: number | undefined) { return v != null ? `${(v * 100).toFixed(1)}%` : "—"; }

export default function UsEtfDetailPage({ params }: { params: { symbol: string } }) {
  const symbol = params.symbol.toUpperCase();
  const { data, isLoading, error } = useSWR(`us-etf-${symbol}`, () => fetchUsEtfDetail(symbol));
