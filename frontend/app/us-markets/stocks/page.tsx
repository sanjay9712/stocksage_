...
import { fetchUsStockScreen, refreshUsStockScreen, type UsStockScreen } from "@/lib/api";
import UsStockSearch from "@/components/UsStockSearch";
...
export default function UsStocksPage() {
  const { data, isLoading, error, mutate } = useSWR("us-stock-screen", fetchUsStockScreen, {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-semibold text-slate-100">US Stocks — Top Picks</h1>
      ...
      {data && rows.length > 0 && (
        <div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 fade-in">
            {rows.map((s: UsStockScreen, idx) => (
              <Link key={s.symbol} href={`/us-markets/stock/${s.symbol}`} className="glass-card-hover p-4">
                ...
                <div className="grid grid-cols-4 gap-2 mb-3">
                  ...
                  <div className="text-center">
                    <div className="text-[10px] text-slate-400 uppercase">RS vs SPY</div>
      ...
      <p className="text-xs text-slate-600 text-center">
        Screened via multi-factor score (momentum 50% · quality 30% · value 20%). US data via yfinance (delayed ~15 min). Not investment advice.
      </p>
    </div>
  );
}
