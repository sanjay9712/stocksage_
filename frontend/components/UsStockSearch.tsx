...
import { fetchUsStockSearch, fetchUsStockSuggest, type UsStockSearchResult, type UsStockSuggestion } from "@/lib/api";

export default function UsStockSearch() {
  const [query, setQuery] = useState("");
  const [suggestions, setSuggestions] = useState<UsStockSuggestion[]>([]);
  const [showDropdown, setShowDropdown] = useState(false);
  const [highlightIdx, setHighlightIdx] = useState(-1);
  const [result, setResult] = useState<UsStockSearchResult | null>(null);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const r = await fetchUsStockSuggest(query.trim());
  ...
  function selectSuggestion(s: UsStockSuggestion) {
  ...
  async function handleSearch(e?: React.FormEvent, forcedSymbol?: string) {
    ...
    setShowDropdown(false);
    try {
      const r = await fetchUsStockSearch(sym);
      setResult(r);
    } catch {
        ...
        <div className="flex gap-2 items-stretch">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search any US stock — AAPL, MSFT, GOOGL..."
      ...
      {result && result.found && result.quote && (
        <div className={`p-4 rounded-lg border border-slate-800/60 bg-slate-800/30 fade-in space-y-3 ${showDropdown && suggestions.length > 0 ? "mt-[20.5rem]" : "mt-4"}`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              {result.fundamentals.market_cap != null && (
                <Stat label="Mkt Cap" value={
                  ...
                  result.fundamentals.market_cap >= 1e9 ? `$${(result.fundamentals.market_cap / 1e9).toFixed(0)}B` : "—"
                } />
              )}
            </div>
          )}
          ...
          <div className="flex items-center justify-between pt-1">
            <Link href={`/us-markets/stock/${result.symbol}`} className="text-xs text-sky-400 hover:underline">
              View full details →
            </Link>
            <p className="text-xs text-slate-600">
              via yfinance · may be delayed ~15 min
            </p>
          </div>
        </div>
      )}
